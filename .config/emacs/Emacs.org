...
srefactor is a package which adds the ablility to do things like inserting C++ member functions from a class header.

TODO create function that enables ~semantic-mode~, runs ~srefactor-refactor-at-point~, then disables ~semantic-mode~
...
*** C/C++
Not much going on here right now. We are basically using ccls as our language server and lsp-mode handles the rest. Emacs does have built-in modes for C/C++ files (I think cuda-mode is not built-in though).
